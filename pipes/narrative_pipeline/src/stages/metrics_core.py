...
STOPWORDS = {
    ...
    "has", "had", "because", "while", "through", "against", "inside", "outside", "like", "what",
    "said", "them", "each", "one", "other", "sometimes", "didnt", "dont", "cant", "wont",
    "youre", "thats", "theres",
}
THEME_BLOCKLIST = {
    "about", "before", "after", "already", "first", "just", "because", "through", "inside",
    "against", "forward", "back", "really", "maybe", "still", "around", "like", "what",
}
...
def metric_document_themes(paragraphs: list[str], tokens: list[str]) -> dict[str, Any]:
    paragraph_tokens = [content_tokens(tokenize(paragraph)) for paragraph in paragraphs]
    ...
    for phrase, count in phrase_counts.items():
        ...
        rows.append({"theme": phrase, "count": count, "paragraph_spread": spread, "type": "phrase", "score": round(score, 4)})
    rows.sort(key=lambda row: row["score"], reverse=True)
    top = rows[:20]
    highlights = []
    if top:
        highlights.append({"kind": "themes", "severity": "info", "message": f"Top recurring theme marker: {top[0]['theme']}."})
    return {"summary": {"theme_count": len(top)}, "themes": top, "highlights": highlights}


def metric_document_patterns(paragraphs: list[str]) -> dict[str, Any]:
    paragraph_tokens = [tokenize(paragraph) for paragraph in paragraphs]
    bigrams = Counter()
    trigrams = Counter()
    ...
    highlights = []
    if top_trigrams:
        highlights.append({"kind": "patterns", "severity": "low", "message": f"Most repeated long pattern: '{top_trigrams[0]['pattern']}' ({top_trigrams[0]['count']}x)."})
    return {"summary": {"repeated_bigram_count": len(top_bigrams), "repeated_trigram_count": len(top_trigrams)}, "top_bigrams": top_bigrams, "top_trigrams": top_trigrams, "highlights": highlights}


def metric_document_entropy(tokens: list[str]) -> dict[str, Any]:
    return {"summary": {"document_entropy": entropy(tokens)}, "highlights": []}


def metric_paragraph_signal_density(paragraphs: list[str]) -> dict[str, Any]:
    rows = []
    for idx, paragraph in enumerate(paragraphs, start=1):
        tokens = tokenize(paragraph)
        lexical = content_tokens(tokens)
        density = (len(lexical) / len(tokens)) if tokens else 0.0
        rows.append({"paragraph_id": f"p-{idx:04d}", "token_count": len(tokens), "lexical_density": round(density, 4)})
    ...
    highlights = []
    if low_rows:
        highlights.append({"kind": "signal_density", "severity": "medium", "message": f"{len(low_rows)} paragraphs have low lexical density (<0.45).", "anchors": {"paragraph_id": low_rows[0]["paragraph_id"]}})
    return {"summary": {"average_lexical_density": avg, "low_density_paragraph_count": len(low_rows)}, "paragraphs": rows, "lowest_density_paragraphs": low_rows[:12], "highlights": highlights}


def metric_paragraph_surprisal(paragraphs: list[str], tokens: list[str]) -> dict[str, Any]:
    ...
    for idx, paragraph in enumerate(paragraphs, start=1):
        ...
        if len(paragraph_tokens) < MIN_PARAGRAPH_TOKENS:
            skipped_short += 1
        paragraph_rows.append({"paragraph_id": f"p-{idx:04d}", "token_count": len(paragraph_tokens), "surprisal": round(raw, 4), "adjusted_surprisal": round(adjusted, 4)})
    ...
    if ranked:
        top = ranked[0]
        highlights.append({"kind": "surprisal", "severity": "low", "message": f"Highest adjusted surprisal is {top['paragraph_id']} ({top['adjusted_surprisal']}).", "anchors": {"paragraph_id": top["paragraph_id"]}})
    return {"summary": {"max_adjusted_surprisal": ranked[0]["adjusted_surprisal"] if ranked else 0.0, "mean_adjusted_surprisal": mean, "short_paragraphs_excluded_from_ranking": skipped_short}, "top_paragraphs": ranked[:12], "all_paragraphs": paragraph_rows, "highlights": highlights}


def metric_paragraph_entropy(paragraphs: list[str]) -> dict[str, Any]:
    paragraph_rows = []
    for idx, paragraph in enumerate(paragraphs, start=1):
        paragraph_tokens = tokenize(paragraph)
        value = entropy(paragraph_tokens)
        vocab_size = len(set(paragraph_tokens))
        max_entropy = math.log2(vocab_size) if vocab_size > 1 else 0.0
        normalized = (value / max_entropy) if max_entropy > 0 else 0.0
        paragraph_rows.append({"paragraph_id": f"p-{idx:04d}", "token_count": len(paragraph_tokens), "entropy": value, "normalized_entropy": round(normalized, 4)})
    ranked = [row for row in paragraph_rows if row["token_count"] >= MIN_PARAGRAPH_TOKENS]
    ranked.sort(key=lambda row: row["normalized_entropy"])
    low_variety = ranked[:10]
    highlights = []
    if low_variety:
        highlights.append({"kind": "entropy", "severity": "low", "message": f"Lowest normalized entropy appears at {low_variety[0]['paragraph_id']}.", "anchors": {"paragraph_id": low_variety[0]["paragraph_id"]}})
    return {"summary": {"evaluated_paragraph_count": len(ranked)}, "paragraphs": paragraph_rows, "lowest_variety_paragraphs": low_variety, "highlights": highlights}


def metric_hybrid_semantic_repetition(paragraphs: list[str]) -> dict[str, Any]:
    paragraph_tokens = [content_tokens(tokenize(paragraph)) for paragraph in paragraphs]
    ...
    for members in cluster_rows:
        ...
        top_clusters.append({"paragraph_ids": member_ids, "cluster_size": len(members), "shared_tokens": shared_tokens, "example": paragraphs[members[0]][:220]})
    top_clusters.sort(key=lambda row: row["cluster_size"], reverse=True)
    highlights = []
    if top_clusters:
        highlights.append({"kind": "semantic_repetition", "severity": "medium", "message": f"{len(top_clusters)} near-duplicate paragraph clusters detected.", "anchors": {"paragraph_id": top_clusters[0]["paragraph_ids"][0]}})
    return {
        "summary": {
        ...
        "top_repeats": [{"text": row["example"], "count": row["cluster_size"]} for row in top_clusters[:12]],
        "top_clusters": top_clusters[:12],
        "highlights": highlights,
    }


def metric_hybrid_burstiness(paragraphs: list[str]) -> dict[str, Any]:
    paragraph_tokens = [content_tokens(tokenize(paragraph)) for paragraph in paragraphs]
    document_frequency = Counter()
    for tokens in paragraph_tokens:
        document_frequency.update(set(tokens))
    paragraph_count = max(len(paragraphs), 1)
    candidate_tokens = [token for token, df in document_frequency.items() if df >= 4 and df <= int(paragraph_count * 0.35)]
    ...
    if token_occurrence_rows:
        ...
        highlights.append({"kind": "burstiness", "severity": "low", "message": f"Burst token '{top['token']}' in {top['peak_paragraph_id']}.", "anchors": {"paragraph_id": top["peak_paragraph_id"]}})
    return {"summary": {"burst_token_count": len(token_occurrence_rows)}, "bursts": token_occurrence_rows[:25], "highlights": highlights}
