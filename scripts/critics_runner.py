...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Load each markdown critic prompt from prompts/critics, query the local LLM, "
            "and write one unstructured JSON object keyed by critic filename."
        )
    )
...
def call_lm(base_url: str, model: str, prompt_text: str, timeout: int) -> str:
    payload = {
        "model": model,
        "messages": [
            {
...
def main() -> None:
    ...
    for critic_file in critics:
        ...
        prompt_text = critic_file.read_text(encoding="utf-8")
        response_text = call_lm(args.base_url, args.model, prompt_text, args.timeout)
