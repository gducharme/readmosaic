...
def main() -> int:
    ...
    for edit in edits:
        if edit.status in {"PENDING", "REGENERATING"} and not edit.last_refined:
            paragraphs = split_paragraphs(manuscript)
            try:
                paragraph_index, _ = locate_paragraph(paragraphs, edit.search_anchor)
            except ValueError:
                continue
            edit.status = "REGENERATING"
            futures[edit.edit_id] = submit_refinement(
                edit, paragraphs[paragraph_index]
            )
    ...
    for edit in edits:
        ...
        if edit.last_refined:
            ...
            if edit.edit_id not in futures:
                edit.status = "REGENERATING"
        ...
        for upcoming in edits:
            if (
                upcoming.status in {"PENDING", "REGENERATING"}
                and upcoming.edit_id not in futures
                and not upcoming.last_refined
            ):
